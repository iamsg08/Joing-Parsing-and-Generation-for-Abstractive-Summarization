...
def test(config):
    options = optionsLoader(LOG, config.optionsFrame, disp=True)
    Vocab = loadFromPKL("others/vocab/"+config.data+".Vocab")
    Best_Model = torch.load("others/model/"+config.model + "_" + config.data + "/model_best.pth.tar", map_location =torch.device('cpu'))
