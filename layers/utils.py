...
def unpack2Dto3D(M, lengths):
    ...
    for i in range(n_batch):
        ...
        seq = M[start:end,:]
        if lengths[i] != maxLength:
            seq = torch.cat([seq, Variable(torch.zeros((maxLength - lengths[i], dim)), requires_grad = False)], dim = 0)
                                                                                                        #false).cuda
        seqs.append(seq)
        start = end
...
def packList2Dto3D(seqs):
    ...
    for seq in seqs:
        if seq.size()[0] == maxLength:
            ...
            result.append(torch.cat([seq, Variable(torch.zeros((maxLength - seq.size()[0], seq.size()[1])), requires_grad = False)], dim = 0))
            # #false).cuda
